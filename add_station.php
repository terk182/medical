...
<div class="dashboard-wrapper">
    <div class="container-fluid dashboard-content">
        ...
        $sql_cl = "SHOW COLUMNS FROM  `$database`.`station`";
        ...
        $db_select = mysqli_select_db($conn, `station`);
        $sql = "SELECT process_id FROM station ORDER BY id DESC LIMIT 1";
                                            ...
                                            <Table class="table table-striped table-bordered first">
                                                <tr>
                                                    <td colspan="2">
                                                        ...
                                                        <input type="submit" name="submit" value="Add Station" class="btn btn-warning" onclick="return confirm('คุณต้องการแก้ไขข้อมูลจริงๆใชมั้ย ?')" />
                                                    </td>
                                                </tr>
                                                ...
                                                <tr>
                                                    <td colspan="2">
                                                        <input type="hidden" name="id" value="<?php echo $id; ?>">
                                                        <input type="submit" name="submit" value="Add process" class="btn btn-warning" onclick="return confirm('คุณต้องการแก้ไขข้อมูลจริงๆใชมั้ย ?')" />
                                                    </td>
                                                </tr>
                    ...
                    <div class="col-xl- col-lg-6 col-md-12 col-sm-12 col-12">
                        <div class="card">
                            <div class="card-header bg-warning text-center p-3">
                                <h4 class="mb-0 text-white">Add station detail</h4>
                            </div>
...
if (isset($_POST['submit'])) {
    ...
    if (!empty($detail)) {
        ...
        $sql_insert = "INSERT INTO `$database`.`station`";
        $column ="";
        $value ="";
        for ($x = 0; $x <= sizeof($stack) - 1; $x++) {
       ...
       // echo $sql_insert;
        ...
        if ($res == true) {
            ...
            $_SESSION['update'] = "<div class = 'success'>Add station Successfully.</div>";
            audit_t($OperatorName, $OperatorID, "position= $id", "add station", "add station", "$detail add station $process", $ECN_No,$get_database);
            echo '<script type="text/javascript">';
            echo ' alert("add station complete")';  //not showing an alert box.
            echo '</script>';
        ...
        echo '<script type="text/javascript">';
        echo ' alert("ต้องใส่เหตุผลในการ add station ด้วย")';  //not showing an alert box.
        echo '</script>';
    }
}
